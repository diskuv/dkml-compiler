...
if [ "$BYTECODEONLY" = OFF ] && [ -n "$FLEXLINK_CHAIN" ]; then
    hermetic_util cp -p "boot/flexdll_initer_${FLEXLINK_CHAIN}${FLEXLINK_EXT}" "$OCAMLHOST_UNIX"/bin/
    hermetic_util cp -p "boot/flexdll_${FLEXLINK_CHAIN}${FLEXLINK_EXT}" "$OCAMLHOST_UNIX"/bin/
fi
